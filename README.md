# biblecoin-client-binaries

[![Build Status](https://secure.travis-ci.org/biblecoin/biblecoin-client-binaries.png?branch=master)](http://travis-ci.org/biblecoin/biblecoin-client-binaries) [![NPM module](https://badge.fury.io/js/biblecoin-client-binaries.png)](https://badge.fury.io/js/biblecoin-client-binaries) [![Twitter URL](https://img.shields.io/twitter/url/http/shields.io.svg?style=social&label=Follow&maxAge=2592000)](https://twitter.com/biblecoin)

Download Biblecoin client binaries for your OS.

When you wish to run a local Biblecoin client node it would be beneficial to first 
...
Features:
* Configurable client types (biblecoind, Eth, Parity, etc)
...
```shell
npm install --save biblecoin-client-binaries
```
...
For example, a config object which specifies the [biblecoind client](https://github.com/biblecoin/go-biblecoin) for only 64-bit Linux platforms and the [Parity client](https://github.com/ethcore/parity) for only 32-bit Windows platforms might be:
    ...
    "biblecoind": {
      "platforms": {
        "linux": {
          "x64": {
            "download": {
              "url": "https://BibleCoind.com/latest.tgz",
              "type": "tar",
              "bin": "BibleCoind-linux-x64",
...
The `sanity` command is mandatory and is a way to check a found binary to ensure that is is actually a valid client binary and not something else. In the above config the `sanity` command denotes that running `BibleCoind version` should return output containing *both* `biblecoind` and `1.4.12`.

Now we can construct a `Manager` with this config:

```js
const Manager = require('biblecoin-client-binaries').Manager;

// construct
const mgr =  new Manager(config);
```

**Note:** If no config is provided then the default config ([src/config.json](https://github.com/biblecoin/biblecoin-client-binaries/blob/master/src/config.json)) gets used.
...
Let's say the current platform is `linux` with an `x64` architecture, and that `BibleCoind` has been resolved successfully to `/usr/local/bin/BibleCoind`, the `mgr.clients` property will look like:
...
[
  {
    ...
    activeCli: {
      ...
      fullPath: '/usr/local/bin/BibleCoind'
    }
  }
]
*/
...
1. `notFound` - a binary with matching name (`BibleCoind` in above example) could not be found.
...
`https://biblecoin.org/` are valid and allowed.
